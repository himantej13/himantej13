...
const setMessage = (type, element, message = "") => {
  ...
  const errorField = inputGroup.querySelector(".error-field");
...
const validateForm = () => {
  ...
  // check user name field for error and success
    if (userNameValue === "") {
        ...
        setMessage(messageType.ERROR, userName,"User Name should contain 3-20 characters");
        isFormValid = false;
    } else {
  ...
  // check phone number field for error and success
    if (phoneValue === "") {
        ...
        isFormValid = false;
    } else if (phoneValue.length !== 10) {setMessage(messageType.ERROR, phone,"Phone number should contain 10 digits");
        isFormValid = false;
    } else if (!isNumeric(phoneValue)) {
        setMessage(messageType.ERROR,phone,"Phone number should only contain digits");
        isFormValid = false;
    } else {
